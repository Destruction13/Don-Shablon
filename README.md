...
2. **Установите зависимости**
   ...
   ```
   > OCR теперь использует библиотеку **PaddleOCR** 3.0.1. Модели уже находятся
   > в каталоге `data/ocr_models`, поэтому скачивание не требуется.
...
В `logic/ocr_paddle.py` можно настроить пороги распознавания:

- `SCORE_IGNORE_THRESHOLD` – строка пропускается, если уверенность ниже значения (по умолчанию 0.7);
- `SCORE_THRESHOLD` – минимальная уверенность для учёта при парсинге (0.9);
